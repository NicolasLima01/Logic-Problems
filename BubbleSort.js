...
function BubbleSort(...args) { //allows the function to treat an indefinite number of arguments as an array 
    let leng = args.length; //length of the array
    for (let i = 0; i < leng; i++) { //loop to iterate(repeat) through the array
        for (let j = 0; j < leng; j++) { //loop to compare the elements of the array
            if (args[j + 1] < args[j]) {
                let value = args[j + 1];
                args[j + 1] = args[j];
                args[j] = value;
